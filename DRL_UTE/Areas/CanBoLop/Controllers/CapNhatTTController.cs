...
namespace DRL_UTE.Areas.CanBoLop.Controllers
{
    public class CapNhatTTController : BaseController
    {
        ...
        [HandleError]
        [HttpPost]
        public ActionResult Index(tblSinhVien editPro, HttpPostedFileBase image)
        {
            var session = (UserLogin)Session[Constants.USER_SESSION];
            if (session == null)
            {
        ...
        public ActionResult UpdateMK(UserUpdateMK userud)
        {
            ...
            else
            {
                var maSV = session.UserName;
                var dao = new SinhVienDao();
                var result = dao.doimatkhau(maSV, userud.matKhau, userud.mk1, userud.mk2);
                if (result == 0)
                {
